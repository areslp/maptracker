...
# 从 mmengine.hooks 导入基本的 Hook 和 LoggerHook
...
class EvalHook(Hook):
    ...
    def __init__(self,
        ...
        if test_fn is None:
            # 从 tools.mmdet_test 导入 single_gpu_test
            from tools.mmdet_test import single_gpu_test
    ...
    def before_train_iter(self, runner, batch_idx: int, data_batch=None):
        ...
        if self.start is not None and runner.iter >= self.start:
            self.after_train_iter(runner, batch_idx, data_batch)
        ...
        if not self.by_epoch and self._should_evaluate(runner):
            ...
            for hook in runner._hooks:
                if isinstance(hook, LoggerHook):
                    hook.after_train_iter(runner, batch_idx, data_batch, outputs)
    ...
    def _should_evaluate(self, runner):
        ...
        if self.by_epoch:
            ...
            current = runner.iter
            check_time = self.every_n_train_iters
...
class DistEvalHook(EvalHook):
    ...
    def __init__(self,
        ...
        if test_fn is None:
            # 使用 plugin.core.apis.test 中的 custom_multi_gpu_test
            from plugin.core.apis.test import custom_multi_gpu_test
            test_fn = custom_multi_gpu_test
